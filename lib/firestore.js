    ...
    try {
      ...
      if (snap.exists()) {
        ...
        if (phone) {
          ...
          const customerName = orderInfo.address?.firstName ? `${orderInfo.address.firstName} ${orderInfo.address.lastName || ''}`.trim() : (orderInfo.customer || "Customer");
...
export async function getWhatsAppConfig() {
  try {
    ...
    return { apiUrl: '', apiKey: '', isEnabled: false };
  } catch (err) {
...
export async function updateWhatsAppConfig(data) {
  try {
    await setDoc(doc(db, 'settings', 'whatsapp'), data, { merge: true });
    return true;
  } catch (err) {
...
export async function sendWhatsAppMessage(number, text) {
  try {
    ...
    if (!config || !config.isEnabled || !config.apiUrl || !config.apiKey) return;
    ...
    // We try to parse json if possible, but API might return text
