...
export async function createOrder(orderData) {
  ...
  if (orderData.phone) {
    try {
      ...
      const customerName = orderData.customer || "Customer";
      ...
      fetch('/api/send-sms', {
        ...
        body: JSON.stringify({
          phone: phone,
          message: `Hi ${customerName}, your Chai Chaska order #${shortId} has been placed successfully! ☕`
        })
      }).catch(err => console.error("SMS notification failed:", err));
    } catch (err) {
      console.error("SMS preparation failed:", err);
    }
  }
  return orderId;
}
    ...
    try {
      ...
      if (snap.exists()) {
        ...
        if (phone) {
          ...
          const customerName = orderInfo.address?.firstName || orderInfo.address?.name || orderInfo.customer || "Customer";
          ...
          fetch('/api/send-sms', {
            ...
            body: JSON.stringify({
              phone: phone,
              message: `Hi ${customerName}, your Chai Chaska order #${(orderInfo.orderId || id).slice(-6).toUpperCase()} status is now: ${data.status}.`
            })
          ...
          // customerName is already declared above
