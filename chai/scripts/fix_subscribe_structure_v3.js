...
const badStart = `                <div
                ...
                >`;
const badEnd = `                <div className="summary-item" style={{ borderTop: "1px dashed rgba(0,0,0,0.08)", paddingTop: "12px", marginTop: "12px" }}>
...
const s = content.indexOf(badStart);
const e = content.indexOf(badEnd) + badEnd.length;

if (s !== -1 && content.indexOf(badEnd) !== -1) {
  content = content.substring(0, s) + goodContent + content.substring(e);
  fs.writeFileSync(path, content);
  console.log("Successfully fixed");
} else {
  console.log("Failed to find bounds. Start index: " + s + " End index: " + content.indexOf(badEnd));
}
