...
const badBlock = `              const activeSubs = subscriptions.filter(s => s.status === "Active");
...
if (content.includes(badBlock) && content.includes(badBlock2)) {
  ...
  console.log("Fixed app/chaimaker/page.js");
} else {
  console.log("Could not find blocks in chaimaker");
  if (!content.includes(badBlock)) {
    console.log("Missing badBlock 1");
    console.log("Trying to find alternative block 1");
  }
